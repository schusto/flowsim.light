...
import { renderConfig } from './ui/config.mjs';
import { renderGrid, renderItemsIntoGrid } from './ui/grid.mjs';

export function addState(name='New State'){ state.states.push({id:uid(), name}); renderConfig(); renderGrid(); saveSnapshot(); }
export function renameState(id, name){ const s=state.states.find(x=>x.id===id); if(!s) return; s.name=name||s.name; renderConfig(); renderGrid(); saveSnapshot(); }
export function deleteState(id){
  ...
  state.states = remaining; renderConfig(); renderGrid(); saveSnapshot();
}
export function moveState(id, delta){
  ...
  const [sp]=state.states.splice(i,1); state.states.splice(j,0,sp); renderConfig(); renderGrid(); saveSnapshot();
}

export function addGroup(name='Workgroup'){ state.groups.push({id:uid(), name}); renderConfig(); renderGrid(); saveSnapshot(); }
export function renameGroup(id, name){ const g=state.groups.find(x=>x.id===id); if(!g) return; g.name=name||g.name; renderConfig(); renderGrid(); saveSnapshot(); }
export function deleteGroup(id){
  ...
  state.groups = remaining; renderConfig(); renderGrid(); saveSnapshot();
}
export function moveGroup(id, delta){
  ...
  const [gp]=state.groups.splice(i,1); state.groups.splice(j,0,gp); renderConfig(); renderGrid(); saveSnapshot();
}
